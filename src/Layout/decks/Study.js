...
function nextButton(index, cards){
    if(!front && index === cards.length - 1) {
        ...
        return (
            ...
            className="btn btn-primary my-2 mx-1" >Next</button>
        );
    } else{
        return null;
    }
    ...
    useEffect(() => {
      ...
      try {
        ...
        console.log("study deck", response);
        setDeck(response);
        setCards(response.cards);
        console.log("study cards", response.cards);
      } catch (error) {
            ...
            return (
              <div className="card" key={card.id}>
                <div className="container">
                  <h5 className="card-title mb-2 mx-1 mt-4">{`Card ${
                    index + 1
                  } of ${cards.length}`}</h5>
